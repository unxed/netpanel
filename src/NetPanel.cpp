...
Network panel main plugin code
...
static TCHAR NetPanelPath[] = WGOOD_SLASH _T("NetPanel");
static wchar_t *NetPanelModule = nullptr;

const wchar_t *GetNetPanelModule()
{
	return NetPanelModule;
}

SHAREDSYMBOL void WINAPI EXP_NAME(SetStartupInfo)(const struct PluginStartupInfo *Info)
{
	fprintf(stderr, "SetStartupInfo\n");
	::Info = *Info;
	::FSF = *Info->FSF;
	::Info.FSF = &::FSF;
	NetPanelModule = wcsdup(Info->ModuleName);

	PluginRootKey = (TCHAR *)malloc((lstrlen(Info->RootKey) + 1) * sizeof(TCHAR) + sizeof(NetPanelPath));
	lstrcpy(PluginRootKey, Info->RootKey);
	lstrcat(PluginRootKey, NetPanelPath);
	GetOptions();
	...
	void ShowMenuFromList(TCHAR * Name)
	{
		...
		if (fmi) {
			...
			if ((unsigned)ExitCode < (unsigned)argc) {
				...
				if (!bShellExecute) {
					if (NetPanel::GetFileInfoAndValidate(p, &FindData, FALSE)) {
...
#undef pName
	...
	{
		fprintf(stderr, "ClosePlugin\n");
		delete (NetPanel *)hPlugin;
	}

	SHAREDSYMBOL void WINAPI EXP_NAME(ExitFAR)()
	{
		fprintf(stderr, "ExitFAR\n");
		free(PluginRootKey);
	...
	{
		fprintf(stderr, "GetFindData api\n");
		NetPanel *Panel = (NetPanel *)hPlugin;
	...
	{
//		fprintf(stderr, "GetOpenPluginInfo api\n");
		NetPanel *Panel = (NetPanel *)hPlugin;
	...
	{
		fprintf(stderr, "SetDirectory api\n");
		NetPanel *Panel = (NetPanel *)hPlugin;
	...
	SHAREDSYMBOL int WINAPI
	EXP_NAME(GetFiles)(HANDLE hPlugin, struct PluginPanelItem * PanelItem, int ItemsNumber, int Move,
			const wchar_t **DestPath,
			int OpMode)
	{
		fprintf(stderr, "GetFiles / %ls / %ls\n", *DestPath, PanelItem->FindData.lpwszFileName);
    ...
    */
	...
	{
		fprintf(stderr, "GetMinFarVersion\n");
		return FARMANAGERVERSION;
	}

	SHAREDSYMBOL int WINAPI EXP_NAME(DeleteFiles)(HANDLE hPlugin,struct PluginPanelItem *PanelItem,int ItemsNumber,int OpMode)
	{
		fprintf(stderr, "DeleteFiles api\n");
		NetPanel *Panel = (NetPanel *)hPlugin;
		return (Panel->DeleteFiles(PanelItem,ItemsNumber,OpMode));
	}

	SHAREDSYMBOL int WINAPI EXP_NAME(MakeDirectory)(HANDLE hPlugin,const wchar_t **Name,int OpMode) {
		fprintf(stderr, "MakeDirectory api\n");
		NetPanel *Panel = (NetPanel *)hPlugin;
		return (Panel->MakeDirectory(Name,OpMode));
	}
