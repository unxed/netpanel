...
#define INI_LOCATION InMyConfig("plugins/netpanel/config.ini")
...
int Config()
{
	...
	if (GetNetPanelModule()) {
		// plugins menus could change - purge all information cached by far2l
		Info.PluginsControl(NULL, PCTL_CACHEFORGET, PLT_PATH, (LONG_PTR)GetNetPanelModule());
	}
done:
#ifdef UNICODE
